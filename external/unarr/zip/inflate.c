...
static __forceinline uint64_t br_bits_zip(inflate_state *state, int bits)
{
...
static __forceinline int tree_get_value(inflate_state *state, const struct tree *tree, bool not_fast)
{
    if (state->state.tree_idx == 0) {
        ...
        if (tree->nodes[key].is_value) {
            ...
            (void)br_bits_zip(state, tree->nodes[key].length);
            return RESULT_EOS;
        }
        if (tree->nodes[key].length == 0)
            return RESULT_ERROR;
        (void)br_bits_zip(state, TREE_FAST_BITS);
    ...
    while (state->state.value == -1) {
        ...
        if (not_fast && !br_ensure(state, 1))
            return RESULT_NOT_DONE;
        idx = state->state.tree_idx | (int)br_bits_zip(state, 1);
    ...
    for (;;) {
        ...
        case STEP_NEXT_BLOCK:
            ...
            if (!br_ensure(state, 3))
                return RESULT_NOT_DONE;
            state->is_final_block = br_bits_zip(state, 1) != 0;
            switch (br_bits_zip(state, 2)) {
            case 0:
        ...
        case STEP_COPY_INIT:
            if (!br_ensure(state, 32))
                return RESULT_NOT_DONE;
            (void)br_bits_zip(state, state->in.available & 0x7);
            state->state.length = (uint16_t)br_bits_zip(state, 16);
            if (state->state.length != 0xFFFF - (uint16_t)br_bits_zip(state, 16))
                return RESULT_ERROR;
        ...
        case STEP_COPY:
            while (state->state.length > 0) {
                if (!br_ensure(state, 8) || *avail_out == 0)
                    return RESULT_NOT_DONE;
                output(state, (uint8_t)br_bits_zip(state, 8));
                state->state.length--;
            }
        ...
        case STEP_INFLATE:
            ...
            state->state.length = table_lengths[state->state.value - 257].length + (int)br_bits_zip(state, table_lengths[state->state.value - 257].bits);
            state->state.value = -1;
            /* fall through */
        ...
        case STEP_INFLATE_DISTANCE:
            ...
            state->state.dist = table_dists[state->state.value].dist + (int)br_bits_zip(state, table_dists[state->state.value].bits);
        ...
        case STEP_INFLATE_DYNAMIC_INIT:
            if (!br_ensure(state, 14))
                return RESULT_NOT_DONE;
            state->prepare.hlit = (int)br_bits_zip(state, 5) + 257;
            state->prepare.hdist = (int)br_bits_zip(state, 5) + 1;
            state->prepare.hclen = (int)br_bits_zip(state, 4) + 4;
        ...
        case STEP_INFLATE_DYNAMIC_INIT_PRETREE:
            while (state->prepare.idx < state->prepare.hclen) {
                if (!br_ensure(state, 3))
                    return RESULT_NOT_DONE;
                state->prepare.clens[table_code_length_idxs[state->prepare.idx]] = (int)br_bits_zip(state, 3);
                state->prepare.idx++;
            }
        ...
        case STEP_INFLATE_DYNAMIC_INIT_TREES:
            while (state->prepare.idx < state->prepare.hlit + state->prepare.hdist) {
                ...
                if (state->state.value < 16) {
                    ...
                    repeat = (int)br_bits_zip(state, 2) + 3;
                }
                else if (state->state.value == 17) {
                    if (!br_ensure(state, 3))
                        return RESULT_NOT_DONE;
                    value = 0;
                    repeat = (int)br_bits_zip(state, 3) + 3;
                }
                else {
                    if (!br_ensure(state, 7))
                        return RESULT_NOT_DONE;
                    value = 0;
                    repeat = (int)br_bits_zip(state, 7) + 11;
                }
                if (repeat) {
