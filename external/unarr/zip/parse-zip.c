...
static inline uint16_t uint16le_zip(unsigned char *data) { return data[0] | data[1] << 8; }
static inline uint32_t uint32le_zip(unsigned char *data) { return data[0] | data[1] << 8 | data[2] << 16 | data[3] << 24; }
static inline uint64_t uint64le(unsigned char *data) { return (uint64_t)uint32le_zip(data) | (uint64_t)uint32le_zip(data + 4) << 32; }
...
static bool zip_parse_extra_fields(ar_archive_zip *zip, struct zip_entry *entry)
{
    ...
    for (idx = 0; idx + 4 < entry->extralen; idx += 4 + uint16le_zip(&extra[idx + 2])) {
        if (uint16le_zip(&extra[idx]) == 0x0001) {
            uint16_t size = uint16le_zip(&extra[idx + 2]);
            uint16_t offset = 0;
            ...
            if (entry->disk == UINT16_MAX && offset + 4 <= size) {
                entry->disk = uint32le_zip(&extra[idx + 4 + offset]);
                offset += 4;
            }
            break;
        }
...
bool zip_parse_local_file_entry(ar_archive_zip *zip, struct zip_entry *entry)
{
    ...
    entry->signature = uint32le_zip(data + 0);
    entry->version = uint16le_zip(data + 4);
    entry->flags = uint16le_zip(data + 6);
    entry->method = uint16le_zip(data + 8);
    entry->dosdate = uint32le_zip(data + 10);
    entry->crc = uint32le_zip(data + 14);
    entry->datasize = uint32le_zip(data + 18);
    entry->uncompressed = uint32le_zip(data + 22);
    entry->namelen = uint16le_zip(data + 26);
    entry->extralen = uint16le_zip(data + 28);
...
off64_t zip_find_next_local_file_entry(ar_stream *stream, off64_t offset)
{
    ...
    while (count >= ZIP_LOCAL_ENTRY_FIXED_SIZE) {
        for (i = 0; i < count - 4; i++) {
            if (uint32le_zip(data + i) == SIG_LOCAL_FILE_HEADER)
                return offset + i;
        }
...
bool zip_parse_directory_entry(ar_archive_zip *zip, struct zip_entry *entry)
{
    ...
    entry->signature = uint32le_zip(data + 0);
    entry->version = uint16le_zip(data + 4);
    entry->min_version = uint16le_zip(data + 6);
    entry->flags = uint16le_zip(data + 8);
    entry->method = uint16le_zip(data + 10);
    entry->dosdate = uint32le_zip(data + 12);
    entry->crc = uint32le_zip(data + 16);
    entry->datasize = uint32le_zip(data + 20);
    entry->uncompressed = uint32le_zip(data + 24);
    entry->namelen = uint16le_zip(data + 28);
    entry->extralen = uint16le_zip(data + 30);
    entry->commentlen = uint16le_zip(data + 32);
    entry->disk = uint16le_zip(data + 34);
    entry->attr_internal = uint16le_zip(data + 36);
    entry->attr_external = uint32le_zip(data + 38);
    entry->header_offset = uint32le_zip(data + 42);
...
off64_t zip_find_end_of_last_directory_entry(ar_stream *stream, struct zip_eocd64 *eocd)
{
    ...
    for (i = 0; i < eocd->numentries; i++) {
        if (ar_read(stream, data, sizeof(data)) != sizeof(data))
            return -1;
        if (uint32le_zip(data + 0) != SIG_CENTRAL_DIRECTORY)
            return -1;
        if (!ar_skip(stream, uint16le_zip(data + 28) + uint16le_zip(data + 30) + uint16le_zip(data + 32)))
            return -1;
    }
...
bool zip_parse_end_of_central_directory(ar_stream *stream, struct zip_eocd64 *eocd)
{
    ...
    eocd->signature = uint32le_zip(data + 0);
    eocd->diskno = uint16le_zip(data + 4);
    eocd->diskno_dir = uint16le_zip(data + 6);
    eocd->numentries_disk = uint16le_zip(data + 8);
    eocd->numentries = uint16le_zip(data + 10);
    eocd->dir_size = uint32le_zip(data + 12);
    eocd->dir_offset = uint32le_zip(data + 16);
    eocd->commentlen = uint16le_zip(data + 20);
    ...
    if (ar_read(stream, data, 20) != 20)
        return false;
    if (uint32le_zip(data + 0) != SIG_END_OF_CENTRAL_DIRECTORY_64_LOCATOR)
        return true;
    if ((eocd->diskno != UINT16_MAX && uint32le_zip(data + 4) != eocd->diskno) || uint32le_zip(data + 16) != 1) {
    ...
    eocd->signature = uint32le_zip(data + 0);
    eocd->version = uint16le_zip(data + 12);
    eocd->min_version = uint16le_zip(data + 14);
    if (eocd->diskno == UINT16_MAX)
        eocd->diskno = uint32le_zip(data + 16);
    if (eocd->diskno_dir == UINT16_MAX)
        eocd->diskno_dir = uint32le_zip(data + 20);
...
off64_t zip_find_end_of_central_directory(ar_stream *stream)
{
    ...
    while (fromend < UINT16_MAX + ZIP_END_OF_CENTRAL_DIR_SIZE && fromend < filesize) {
        ...
        for (i = count - ZIP_END_OF_CENTRAL_DIR_SIZE; i >= 0; i--) {
            if (uint32le_zip(data + i) == SIG_END_OF_CENTRAL_DIRECTORY)
