...
static const UInt16 kInitBinEsc8[] = { 0x3CDD, 0x1F3F, 0x59BF, 0x48F3, 0x64A1, 0x5ABC, 0x6632, 0x6051};
...
typedef CPpmd8_Context * CTX_PTR8;
...
static void *AllocUnits8Rar8e(CPpmd8 *p, unsigned indx)
{
  unsigned i;
  void *retVal;
  if (p->GlueCount == 0)
...
static void *AllocUnits8(CPpmd8 *p, unsigned indx)
{
  ...
  return AllocUnits8Rar8e(p, indx);
}
...
static void RestartModel8(CPpmd8 *p)
{
  ...
  p->MinContext = p->MaxContext = (CTX_PTR8)(p->HiUnit -= UNIT_SIZE); /* AllocContext(p); */
  ...
  for (i = m = 0; m < 25; m++)
  {
    ...
    for (k = 0; k < 8; k++)
    {
      UInt16 val = (UInt16)(PPMD_BIN_SCALE - kInitBinEsc8[k] / (i + 1));
...
static void Refresh(CPpmd8 *p, CTX_PTR8 ctx, unsigned oldNU, unsigned scale)
{
...
static void SwapStates8(CPpmd_State *t1, CPpmd_State *t2)
{
  CPpmd_State tmp = *t1;
  *t1 = *t2;
  *t2 = tmp;
...
static CPpmd_Void_Ref CutOff(CPpmd8 *p, CTX_PTR8 ctx, unsigned order)
{
  int i;
  unsigned tmp;
  CPpmd_State *s;
...
#ifdef PPMD8_FREEZE_SUPPORT
static CPpmd_Void_Ref RemoveBinContexts(CPpmd8 *p, CTX_PTR8 ctx, unsigned order)
{
  CPpmd_State *s;
  if (!ctx->NumStats)
  {
...
static void RestoreModel(CPpmd8 *p, CTX_PTR8 c1
    #ifdef PPMD8_FREEZE_SUPPORT
    , CTX_PTR8 fSuccessor
    #endif
    )
...
static CTX_PTR8 CreateSuccessors8(CPpmd8 *p, Bool skip, CPpmd_State *s1, CTX_PTR8 c)
{
  CPpmd_State upState;
  Byte flags;
  ...
  {
    /* Create Child */
    CTX_PTR8 c1; /* = AllocContext(p); */
    if (p->HiUnit != p->LoUnit)
      c1 = (CTX_PTR8)(p->HiUnit -= UNIT_SIZE);
    else if (p->FreeList[0] != 0)
      c1 = (CTX_PTR8)RemoveNode8(p, 0);
    else
    {
      c1 = (CTX_PTR8)AllocUnits8Rar8e(p, 0);
      if (!c1)
        return NULL;
    }
    c1->NumStats = 0;
...
static CTX_PTR8 ReduceOrder(CPpmd8 *p, CPpmd_State *s1, CTX_PTR8 c)
{
  CPpmd_State *s = NULL;
  CTX_PTR8 c1 = c;
...
static void UpdateModel8(CPpmd8 *p)
{
  ...
  {
    CTX_PTR8 cs = CreateSuccessors8(p, True, s, p->MinContext);
    if (cs == 0)
    {
  ...
  *p->Text++ = p->FoundState->Symbol;
  ...
  if (!fSuccessor)
  {
    CTX_PTR8 cs = ReduceOrder(p, s, p->MinContext);
    if (cs == NULL)
    {
      RESTORE_MODEL(c, 0);
      return;
  ...
  {
    CTX_PTR8 cs = CreateSuccessors8(p, False, s, p->MinContext);
    if (cs == NULL)
    {
      RESTORE_MODEL(c, 0);
      return;
...
{
  CTX_PTR8 c = CTX(SUCCESSOR(p->FoundState));
