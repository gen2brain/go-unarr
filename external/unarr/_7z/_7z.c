...
static void *gSzAlloc7_Alloc7(void *self, size_t size) { (void)self; return malloc(size); }
static void gSzAlloc7_Free7(void *self, void *ptr) { (void)self; free(ptr); }
static ISzAlloc gSzAlloc7 = { gSzAlloc7_Alloc7, gSzAlloc7_Free7 };
...
static void _7z_close(ar_archive *ar)
{
    ...
    free(_7z->entry_name);
    SzArEx_Free(&_7z->data, &gSzAlloc7);
    IAlloc_Free(&gSzAlloc7, _7z->uncomp.buffer);
}
...
static bool _7z_uncompress(ar_archive *ar, void *buffer, size_t buffer_size)
{
    ...
    if (!uncomp->initialized) {
        ...
        SRes res = SzArEx_Extract(&_7z->data, &_7z->look_stream.s, (UInt32)ar->entry_offset, &uncomp->folder_index, &uncomp->buffer, &uncomp->buffer_size, &uncomp->offset, &uncomp->bytes_left, &gSzAlloc7, &gSzAlloc7);
        if (res != SZ_OK) {
...
ar_archive *ar_open_7z_archive(ar_stream *stream)
{
    ...
    res = SzArEx_Open(&_7z->data, &_7z->look_stream.s, &gSzAlloc7, &gSzAlloc7);
    if (res != SZ_OK) {
